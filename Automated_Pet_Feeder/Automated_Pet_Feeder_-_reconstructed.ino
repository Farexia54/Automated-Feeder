...
UltraSonicDistanceSensor distanceSensor(9, 8);  // Initialize ultrasonic sensor that uses digital pins 9 and 8.
...
void setup()
{
  ...
  digitalWrite(DutchTray, LOW);    // Make sure tray is closed
  digitalWrite(redLed, LED_OFF);  // Make sure the red led is off
  digitalWrite(greenLed, LED_OFF);  // Make sure the green led is off
  digitalWrite(blueLed, LED_OFF); // Make sure the blue led is off

  //Protocol Configuration
  Serial.begin(9600);  // Initialize serial communications with PC
  SPI.begin();           // MFRC522 Hardware uses SPI protocol
  mfrc522.PCD_Init();    // Initialize MFRC522 Hardware
  mfrc522.PCD_SetAntennaGain(mfrc522.RxGain_max);   // Ensure the gain is set to max
...
void loop ()
{
  ...
  else 
  {
                              ...
                              mfrc522.uid.uidByte[3] == 0x31))
    {
      ...
      {
        Serial.println(F("I am afraid you are too far away. Please try again."));
      }
     }
    
    if ( findID(readCard) == (mfrc522.uid.uidByte[0] == 0x7A &&       // Check to see if it is Dutch's card
                              mfrc522.uid.uidByte[1] == 0x6E &&
                              mfrc522.uid.uidByte[2] == 0x72 &&
                              mfrc522.uid.uidByte[3] == 0xAE))
    {
        Serial.println(F("Time to fill Dutch's food bowl. Or debug it if you feel like it"));
        DutchGranted(20);         // Impulse that lasts 20 ms
    }

    if (findID(readCard) == (mfrc522.uid.uidByte[0] == 0xEC &&       // Check to see if it matches Midnight's tag
                             mfrc522.uid.uidByte[1] == 0x74 &&
                             mfrc522.uid.uidByte[2] == 0xC9 &&
...
if (findID(readCard) == (mfrc522.uid.uidByte[0] == 0xBA &&       // Check to see if it is Midnight's card
                         mfrc522.uid.uidByte[1] == 0xD4 &&
                         mfrc522.uid.uidByte[2] == 0x66 &&
                         mfrc522.uid.uidByte[3] == 0xAD ))
    {
      Serial.println(F("Time to fill Dutch's food bowl. Or debug it if you feel like it"));
      MidnightGranted(20);         // Impulse that lasts 20 ms
    }
    ...
    {
       Serial.println(F("Who are you and why are you even here?"));
    }
   }
 }
